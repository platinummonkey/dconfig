...
func (s *Store) init() {
	...
	if s.RaftBind == "" {
		s.RaftBind = defaultBind
	}

	if s.RetainSnapshotCount <= 0 {
		s.RetainSnapshotCount = defaultRetainSnapshotCount
	}

	if s.RaftTimeout <= 0 {
		s.RaftTimeout = defaultRaftTimeout
	}
}

// New returns a new Store.
func New(memoryOnly bool) *Store {
	return &Store{
